...
const PROJECT_FILE_EXTENSION = "zoomcue";
const OLD_PROJECT_FILE_EXTENSION = "openscreen";
		...
		async (_, projectData: unknown, suggestedName?: string, existingProjectPath?: string) => {
			try {
				...
				const result = await dialog.showSaveDialog({
					...
					filters: [
						{
							name: mainT("dialogs", "fileDialogs.zoomcueProject"),
							extensions: [PROJECT_FILE_EXTENSION, OLD_PROJECT_FILE_EXTENSION],
						},
